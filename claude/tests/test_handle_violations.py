...
class TestQuarantine:
    """quarantine：原桶 → 隔离桶 + 打 tag + 数据库 blocked=2。"""

    def test_quarantine_happy_path(self, handler):
        handler.minio.object_exists.return_value = True
        records = [{
            'id': 1, 'bucket_name': 'images', 'object_key': 'bad.jpg',
            'violation_type': 'Gamble',
        }]
        stats = handler.quarantine(records, dry_run=False)
        ...
        handler.minio.move_object.assert_called_once_with(
            'images', 'bad.jpg', handler.quarantine_bucket, 'images/bad.jpg'
        )
    ...
    def test_quarantine_source_missing_marks_only(self, handler):
        """源文件已不存在时，只标记数据库不做迁移。"""
    ...
    def test_quarantine_dry_run_does_not_call_minio(self, handler):
        records = [{'id': 1, 'bucket_name': 'images', 'object_key': 'bad.jpg',
                    'violation_type': 'Gamble'}]
        handler.quarantine(records, dry_run=True)
        handler.minio.move_object.assert_not_called()
        handler.minio.set_violation_tag.assert_not_called()
        handler.db.execute_query.assert_not_called()

    def test_quarantine_tag_failure_does_not_fail(self, handler):
    ...
    def test_quarantine_move_failure_no_db_update(self, handler):
...
class TestRestore:
    ...
    def test_restore_happy_path(self, handler):
        ...
        assert stats == {'success': 1, 'failed': 0, 'skipped': 0}
        handler.minio.move_object.assert_called_once_with(
            handler.quarantine_bucket, 'images/falsepos.jpg', 'images', 'falsepos.jpg'
        )
...
class TestDelete:
    ...
    def test_delete_happy_path(self, handler):
        ...
        handler.minio.remove_object.assert_called_once_with(
            handler.quarantine_bucket, 'images/bad.jpg'
        )
        # DB DELETE
